 ...
 * @author A.Egal
 */
public class PluginLoaderActivity extends Activity { // NOPMD by Àðò¸ì on
														// 25.06.13 13:42

	private static final int SPLASHTIME = 2000; // 2 seconds
	public static final int REQUEST_CODE = 0;
	protected transient Handler exitHandler = null;
	protected transient Runnable exitRunnable = null; // NOPMD by Àðò¸ì on
														// 25.06.13 13:42
	...
	private void startDefaultSplashScreen() {
		...
		exitRunnable = new Runnable() { // NOPMD by Àðò¸ì on 25.06.13 13:42
			public void run() {
				exitSplash();
			}
		};
	...
	private void processDataSourceFromPlugin(final Intent data) {
		if (data != null
				&& data.getExtras().getString("resultType")
						.equals("Datasource")) {
			final String[] url = data.getExtras().getStringArray("url");
	...
	private void procesCustomSplashScreen(final Intent data) {
		if (data != null
				&& data.getExtras().getString("resultType")
						.equals("Splashscreen")) {
			loadPlugins();
