...
/**
 * This enum contains the plugin types that can be loaded, they also contain the
 * data needed to succesfully use or load the plugins.
 * 
 * @author A.Egal
 * 
 */
public enum PluginType {
	...
	BOOTSTRAP_PHASE_1() {
		...
		public AbstractConn getPluginConnection() {
			final AbstractConn abstractConn = new BootStrapActivityConnection();
			abstractConn.setPluginType(this);
			return abstractConn;
		}
	...
	MARKER() {
		...
		public AbstractConn getPluginConnection() {
			final AbstractConn abstractConn = new MarkerServiceConnection();
			abstractConn.setPluginType(this);
			return abstractConn;
		}
	...
	DATAHANDLER() {
		...
		public AbstractConn getPluginConnection() {
			final AbstractConn abstractConn = new DataHandlerServiceConnection();
			abstractConn.setPluginType(this);
			return abstractConn;
		}
	...
	/**
	 * Returns the instance of an activity plugin loader that can load activity
	 * plugins
	 */
	public abstract AbstractConn getPluginConnection();
}

/**
 * A loader enum, a activity loader means that the plugin is a activity, and it
 * should be loaded like an activity, A service loader is a plugin that can run
 * in the background and is not visible.
 * 
 * @author A. Egal
 */
enum Loader {
